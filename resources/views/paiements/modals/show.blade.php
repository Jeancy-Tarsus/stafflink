<div class="modal fade"
     id="modalShowPaiement{{ $paiement->id }}"
     tabindex="-1"
     data-backdrop="static"
     data-keyboard="false">

    <div class="modal-dialog modal-xl">

        <div class="modal-content">

            <div class="modal-header bg-info">

                <h5 class="modal-title">

                    <i class="fas fa-eye"></i>

                    Détails du paiement

                </h5>

                <button type="button"
                        class="close"
                        data-dismiss="modal">

                    <span>&times;</span>

                </button>

            </div>

            <div class="modal-body">

                <div class="row">

                    <div class="col-md-6">

                        <table class="table table-bordered">

                            <tr>
                                <th>Référence</th>
                                <td>{{ $paiement->reference }}</td>
                            </tr>

                            <tr>
                                <th>Contrat</th>
                                <td>{{ $paiement->contrat->reference }}</td>
                            </tr>

                            <tr>
                                <th>Client</th>
                                <td>{{ $paiement->contrat->affectation->demande->client->nom }}</td>
                            </tr>

                            <tr>
                                <th>Travailleur</th>
                                <td>
                                    {{ $paiement->contrat->affectation->travailleur->nom }}
                                    {{ $paiement->contrat->affectation->travailleur->prenom }}
                                </td>
                            </tr>

                        </table>

                    </div>

                    <div class="col-md-6">

                        <table class="table table-bordered">

                            <tr>
                                <th>Date</th>
                                <td>{{ $paiement->date_paiement->format('d/m/Y') }}</td>
                            </tr>

                            <tr>
                                <th>Montant payé</th>
                                <td>{{ number_format($paiement->montant,0,',',' ') }} FCFA</td>
                            </tr>

                            <tr>
                                <th>Mode de paiement</th>
                                <td>{{ $paiement->mode_paiement }}</td>
                            </tr>

                            <tr>
                                <th>Référence paiement</th>
                                <td>{{ $paiement->reference_paiement ?: '-' }}</td>
                            </tr>

                        </table>

                    </div>

                </div>

                <hr>

                <div class="row">

                    <div class="col-md-4">

                        <div class="small-box bg-primary">

                            <div class="inner">

                                <h4>

                                    {{ number_format($paiement->contrat->salaire,0,',',' ') }}

                                </h4>

                                <p>

                                    Salaire prévu

                                </p>

                            </div>

                            <div class="icon">

                                <i class="fas fa-money-check"></i>

                            </div>

                        </div>

                    </div>

                    <div class="col-md-4">

                        <div class="small-box bg-success">

                            <div class="inner">

                                <h4>

                                    {{ number_format($paiement->contrat->montant_paye,0,',',' ') }}

                                </h4>

                                <p>

                                    Total payé

                                </p>

                            </div>

                            <div class="icon">

                                <i class="fas fa-wallet"></i>

                            </div>

                        </div>

                    </div>

                    <div class="col-md-4">

                        <div class="small-box bg-danger">

                            <div class="inner">

                                <h4>

                                    {{ number_format($paiement->contrat->reste_a_payer,0,',',' ') }}

                                </h4>

                                <p>

                                    Reste à payer

                                </p>

                            </div>

                            <div class="icon">

                                <i class="fas fa-balance-scale"></i>

                            </div>

                        </div>

                    </div>

                </div>

                <div class="form-group">

                    <label>Observation</label>

                    <textarea class="form-control"
                              rows="3"
                              readonly>{{ $paiement->observation }}</textarea>

                </div>

            </div>

            <div class="modal-footer">

                <button class="btn btn-secondary"
                        data-dismiss="modal">

                    Fermer

                </button>

            </div>

        </div>

    </div>

</div>
